...
Для Контроллеров и Моделей есть пакеты `.controllers` и `.models` соответственно. В пакете `.config` находятся конфигурационные классы. В пакете `.repository` определяются репозитории для взаимодействия с базой данных H2 как `CrudRepository`. В пакете `.utils` находятся дополнительные классы-утилиты.
